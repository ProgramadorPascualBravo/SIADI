...
class SusEnrollmentImport implements ToCollection, WithHeadingRow, WithValidation , SkipsOnFailure, SkipsOnError
{
   use Importable, SkipsErrors, SkipsFailures,LogsTrail;

   public $values;
   private const SUSPENDIDO = 2;
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $key=>$row)
        {
            ...
            if($enroll){
                ...
                $this->sum(true);
                 $enroll->update(['state' => self::SUSPENDIDO]);
            } else {
                $this->sum(false);
            }
